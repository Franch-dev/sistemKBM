...
@section('title', 'Dashboard Staff Secretary')
@section('logo', 'SS')
@section('panel', 'StaffPanel')
@section('heading', 'Dashboard Staff Secretary')

@section('nav')
    <a href="{{ route('staff_secretary.dashboard') }}" class="nav-item active">Dashboard</a>
@endsection
...
@push('scripts')
<script>
    KBM.boot('staff_secretary').then(async (user) => {
        if (!user) return;
